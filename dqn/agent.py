...
class DQNAgent(object):
    def __init__(
        ...
        Arguments:
            ...
            eps_annealing_frames: Number of frames during which epsilon will be annealed
    ...
    def learn(self, batch_size, gamma, frame_number, priority_scale=1.0):
        ...
        Arguments:
            ...
            frame_number: Global frame number (used for calculating importance values)
        ...
        indices = None
        if self.use_per:
            t, importance, indices = self.replay_buffer.get_minibatch(
                batch_size=self.batch_size,
                priority_scale=priority_scale
            )
            (states, actions, rewards, new_states, terminal_flags) = t
    ...
    def save(self, folder_name, save_replay_buffer=False, **kwargs):
        """Saves the Agent and all corresponding properties into a folder
        Arguments:
            folder_name: Folder in which to save the Agent
            save_replay_buffer: whether we should load replay buffer
        ...
        if save_replay_buffer:
            self.replay_buffer.save(os.path.join(folder_name, 'replay_buffer'))

        # Save meta
        with open(os.path.join(folder_name, 'meta.json'), 'w+') as f:
            f.write(json.dumps({
                **{'buff_count': self.replay_buffer.count, 'buff_curr': self.replay_buffer.current},
    ...
    def load(self, folder_name, load_replay_buffer=True):
        ...
        Arguments:
            ...
            load_replay_buffer: whether we should load replay buffer
        Returns:
        ...
        self.target_dqn = tf.keras.models.load_model(os.path.join(folder_name, 'target_dqn.h5'))
        self.optimizer = self.DQN.optimizer

        # Load replay buffer
        if load_replay_buffer:
            self.replay_buffer.load(os.path.join(folder_name, 'replay_buffer'))
