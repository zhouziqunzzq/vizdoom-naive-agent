...
class DQNAgent:
    ...
    def update_target_net(self):
        self.target_net.set_weights(self.dqn.get_weights()[:len(self.target_net.get_weights())])

    def choose_action(self, state, is_training=True):
        if (not is_training) or self.epsilon < np.random.uniform(0, 1):
...
def test(test_episodes_per_epoch, game, agent):
    ...
    for test_episode in trange(test_episodes_per_epoch, leave=False):
        ...
        while not game.is_episode_finished():
            ...
            best_action_index = agent.choose_action(state, is_training=False)
